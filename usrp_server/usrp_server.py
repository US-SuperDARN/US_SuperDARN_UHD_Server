   ...
   def restore_lost_connections(self):
       ...
       for jrad in range(self.RHM.N_RADARs):
          for iUSRP, usrp in enumerate(tmp_address_list[jrad]):
             ...
             try:
                self.logger.debug('Trying to reconnect to USRP {}'.format(tmp_hostname_list[jrad][iUSRP]))
                usrpsock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                usrpsock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
                usrpsock.setsockopt(socket.IPPROTO_TCP, socket.TCP_QUICKACK, 1)
                if self.sock_timeout != None:
...
class ClearFrequencyService():
    ...
    RX_ONLY_SHM_SIZE        = (1 * INT_SIZE)
    FIX_FREQ_SHM_SIZE       = (1 * INT_SIZE)
    ...
    RX_ONLY_SHM_NAME =        "/rx_only"
    FIX_FREQ_SHM_NAME =       "/fix_freq"
    ...
    def __init__(self):
        ...
        try:
            ...
            ClearFrequencyService.shm_objects = [
                ...
                self.create_shm_obj(self.RX_ONLY_SHM_NAME,        self.RX_ONLY_SHM_SIZE       , ),
                self.create_shm_obj(self.FIX_FREQ_SHM_NAME,       self.FIX_FREQ_SHM_SIZE      , ),
    ...
    def premap_shm(self, meta_data=None):
        ...
        if self.shm_objects[0]['shm_ptr'] == None and meta_data != None:
            ...
            # print(f"Mapping {self.shm_objects[9]['name']}")
            self.log.debug("[clearFrequencyService] Verifying Antenna and Sample size")
            self.shm_objects[9]['shm_ptr'] = mmap.mmap(self.shm_objects[9]['shm_fd'], self.shm_objects[9]['size'], mmap.MAP_SHARED, mmap.PROT_READ | mmap.PROT_WRITE)

            # Check if Antenna Num changed, update corresponding values before they're mapped
            shm_ant_num = self.read_m_data(self.shm_objects[9])[0]
    ...
    def send_samples(self, raw_samples, radar_id = 0, fcenter=None, meta_data=None):
        ...
        try:
            ...
            if meta_data is not None:
                ...
                if self.old_meta_data != meta_data_list:
                    ...
                    if self.cur_antenna_num != len(meta_data['antenna_list']) or self.shm_objects[0]['elem_num'] != (len(meta_data['antenna_list']) * int(meta_data['number_of_samples']) * 2):
                        ...
                        self.write_data(self.shm_objects[9], len(meta_data['antenna_list']))
                    ...
                    # print(f"[Frequency Client] Data Write Progress: {self.shm_objects[8]['name']}")

                    # Rearrange meta_data ordering
                    self.write_data(self.shm_objects[8], meta_data_list, 'meta')
            ...
            if raw_samples is not None:
                ...
                self.muted_antennas = []
                self.muted_antennas = self.read_m_data(self.shm_objects[14])
    ...
    def request_clr_freq(self, radar_id, channel_id, beam_num=None, sample_sep=None,
                         clr_range=None, fcenter=None, if_rate=None, rx_only=None, fix_freq=None):
        """ Waits for client requests, then processes server data, writes client
            data, and requests server to process new data. When process is
            terminated, the try/finally block cleans up.\
        ...
        try:
            ...
            for i in range(self.SAMPLE_PARAM_NUM-1, self.SAMPLE_PARAM_NUM + 6):
            ...
            # Write Channel ID
            # print(f"[Frequency Client] Data Write: {self.shm_objects[12]['name']}")
            self.write_data(self.shm_objects[12], channel_id)
            ...
            self.sl_clrfreq['sem'].acquire()
            new_noise_data = []
            new_clrfreq_data = self.read_m_data(self.shm_objects[10])
...
class scanManager():
    ...
    def init_new_scan(self, freq_range_list, scan_beam_list, fixFreq, scan_times_list, scan_duration, integration_duration, start_period, rxonly):
    ...
    def evaluate_clear_freq(self, jrad, cnum, iPeriod, beamNo):
        ...
        clearFreq, noise = self.clearFreqService.request_clr_freq(int(jrad), int(cnum), int(beamNo), int(self.channel.raw_export_data['smsep']), clear_freq_range, int(metaData['usrp_fcenter']), if_rate, int(self.rxonly), int(self.fixFreq))
...
class RadarHardwareManager:
                ...
                if CS_TRIGGER in [ch.active_state for rchans in self.channels for ch in rchans]:
                   ...
                   for ch in np.concatenate(self.channels).tolist():
                      if ch.active_state not in [CS_TRIGGER, CS_INACTIVE]:
                         controlLoop_logger.debug('remaining in TRIGGER because radar {} ch {} state is {} (active swing is {})'.format(ch.rnum, ch.cnum, ch.active_state, ch.swingManager.activeSwing))
    ...
    def initialize_channel(RHM):
        ...
        for channel in newChannelList:
            ...
            RHM.logger.debug('calling CUDA_ADD_CHANNEL at initialize_channel() (radar {} ch {}, swing {}, beam {})'.format(channel.rnum, channel.cnum, channel.swingManager.activeSwing, channel.scanManager.current_beam))
            cmd.transmit()
            cmd_list.append(cmd)
        ...
        for jrad in range(RHM.N_RADARs):
           if radar_active[jrad]:
              RHM.logger.debug('CUDA_GENERATE_PULSE radar {} socket {}'.format(jrad, RHM.cudasocks[jrad]))
    ...
    def trigger_next_swing(self):
        ...
        for ch in np.concatenate(self.channels).tolist():
            self.logger.debug('radar {} ch {}: tfreq={}, rfreq={}, usrp_center={} rx_rate={}MHz'.format(ch.rnum, ch.cnum, ch.ctrlprm_struct.payload['tfreq'], ch.ctrlprm_struct.payload['rfreq'], self.mixingFreqManager.current_mixing_freq[ch.rnum], self.usrp_rf_tx_rate/1e6))
        ...
        for iChannel, channel in enumerate(np.concatenate(self.channels).tolist()):
           self.logger.debug("Checking radar {} ch {} to see if it is processing: state: {}".format(channel.rnum, channel.cnum, channel.processing_state))
           if channel.processing_state is CS_PROCESSING:
        ...
        for channel in np.concatenate(self.channels).tolist():
           self.logger.debug("Checking radar {} ch {} to see if it should be added".format(channel.rnum, channel.cnum))
           if channel.scanManager.isLastPeriod: # or channel.scanManager.isForelastPeriod:
               ...
               self.logger.debug('send CUDA_REMOVE_CHANNEL (radar {} ch {}, swing {})'.format(channel.rnum, channel.cnum, self.swingManager.processingSwing))
               cmd.transmit()
               cmd.client_return()
               ...
               if channel.active:
                  ...
                  self.logger.debug('send CUDA_ADD_CHANNEL (radar {}, ch {}, swing {}, beam {})'.format(channel.rnum, channel.cnum, self.swingManager.processingSwing, channel.scanManager.next_beam))
                  cmd.transmit()
                  cmd.client_return()
        ...
        for jrad in range(self.N_RADARs):
           if self.last_period[jrad]:
              self.logger.debug('skipping CUDA_GENERATE_PULSE radar {} (is last period)'.format(jrad))
           elif radar_active[jrad]:
              self.logger.debug('start CUDA_GENERATE_PULSE radar {}  socket {}'.format(jrad, self.cudasocks[jrad]))
        ...
        for ind, cmd in enumerate(cmd_list):
           ...
           for iUSRP, ready_return in enumerate(payloadList):
              if ready_return != CONNECTION_ERROR:
                 self.usrpManager.pulse_times[jrad] = ready_return["pulse_times"]
                 break
...
# QUICK ACCESS TO CURRENT/NEXT ACTIVE/PROCESSING STATE
    ...
    def close(self):
        self.conn.close()
        self.logger.debug('Deleting radar {} ch {}'.format(self.rnum, self.cnum))
        RHM = self.parent_RadarHardwareManager
        RHM.unregister_channel_from_HardwareManager(self)
        cnum = self.cnum
        rnum = self.rnum
        del self # TODO close thread ?!?
        RHM.logger.info('Deleted radar {} ch {}.'.format(rnum, cnum))
...
#            if state[0] == CS_SAMPLES_READY and counter == 1:
            ...
            if time.time() - wait_start > CHANNEL_STATE_TIMEOUT:
                self.logger.error('CHANNEL STATE TIMEOUT for radar {} ch {}'.format(self.rnum, self.cnum))
                self.close()
                break
...
#        transmit_dtype(self.conn, self.parent_RadarHardwareManager.resultData_nSequences_per_period, np.uint32)
    ...
    def send_results_to_control_program(self):
        ...
        for listno, resultDict in enumerate(self.resultDict_list):
           self.logger.debug("radar {} ch {} list number {}  number of sequences in period: {}".format(self.rnum, self.cnum, listno, resultDict['nSequences_per_period']))
    ...
    def SetActiveHandler(self, rmsg):
        ...
        rxonly = recv_dtype(self.conn, np.int32)
        self.logger.debug('SetActiveHandler rxonly: {}'.format(rxonly))
        ...
        self.scanManager.init_new_scan(freq_range_list, scan_beam_list, fixFreq, scan_times_list, scan_time, integration_time, start_period, rxonly)
        self.triggered_swing_list = []
