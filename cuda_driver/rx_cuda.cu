...
TODO:
 - if filter is real we could save calc time by removing all calculations with
   imag(filter) and phase correction, maybe write additional function multiply_add_real
...
__global__ void multiply_and_add(float *samples, float *odata, float *filter, uint32_t nSamples_if)
{
    ...
    uint32_t iThread_lin = iFilterSampleTimes2 + iChannel*nFilterSamplesDivBy2;

    uint32_t decimationRate_if2bb = decimationRates[1];

    // offset to align center of filter and center of pulse
    ...
    if (iSample_if >= offset) {
        // index in memory (account for I/Q, iChannel, iAntenna)
        uint32_t idxSample_if = (iSample_if - offset) * 2 + iChannel * nSamples_if *2 + iAntenna * nChannels * nSamples_if *2;
        ...
        // zero padding the first part of the filter for output sample 0
        itemp[iThread_lin] = 0;
        qtemp[iThread_lin] = 0;
    }
    __syncthreads();
    ...
    while (s > 0 && rem == 0) {
        s /= 5;
        if (threadIdx.x < s) {
            itemp[iThread_lin] = itemp[iThread_lin] + itemp[iThread_lin + s] +
                                 itemp[iThread_lin+2*s] + itemp[iThread_lin+3*s] +
                                 itemp[iThread_lin+4*s];
            qtemp[iThread_lin] = qtemp[iThread_lin] + qtemp[iThread_lin + s] +
                                 qtemp[iThread_lin+2*s] + qtemp[iThread_lin+3*s] +
                                 qtemp[iThread_lin+4*s];
        }
        rem = s % 5;
...
__global__ void multiply_mix_add(int16_t *samples, float *odata, float *filter, uint32_t nSamples_rf)
{
